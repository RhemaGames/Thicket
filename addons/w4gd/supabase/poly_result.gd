...
const Result = preload("../rest/client_result.gd")
...
class PolyResult extends RefCounted:
	...
	func _get(property: StringName):
		if property == &"_data":
			...
			return _result
		elif property == &"script" or property == &"Built-in script":
			# Avoid breaking debug.
			return get_script()
		elif property == &"RefCounted":
			return null
		var err = "Unknown"
		if is_dict():
...
static func parse_result(result : Result, p_raw:=false) -> PolyResult:
	if result.is_error():
		...
		return PolyResult.new(ResultError.new(result.http_request_result, "The request was cancelled", result), result)
