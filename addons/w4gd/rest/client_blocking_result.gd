...
func wait_request():
	http_client.poll()
	while not is_cancelled() and http_client.get_status() == HTTPClient.STATUS_REQUESTING:
		OS.delay_usec(poll_delay_usec)
		http_client.poll()
...
func read_body() -> PackedByteArray:
	...
	while not is_cancelled() and http_client.get_status() == HTTPClient.STATUS_BODY:
...
func make_request(path : String, headers : PackedStringArray, method : int, raw_data := PackedByteArray()) -> void:
	...
	if is_cancelled() or http_client.get_status() != HTTPClient.STATUS_CONNECTED:
		http_request_result = http_client.get_status()
		result_status = ResultStatus.CANCELLED if is_cancelled() else ResultStatus.ERROR
		return
	...
	if not is_cancelled() or not http_client.has_response():
		http_request_result = http_client.get_status()
		result_status = ResultStatus.CANCELLED if is_cancelled() else ResultStatus.ERROR
		return

	result_status = ResultStatus.DONE
	http_request_result = ResultStatus.CANCELLED if is_cancelled() else HTTPRequest.RESULT_SUCCESS
