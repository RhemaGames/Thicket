...
class Lobby extends RefCounted:
	...
	func subscribe() -> void:
		...
		_lobby_table_synchronizer = TableSynchronizer.new(_client)
		_lobby_table_synchronizer.updated.connect(self._on_lobby_updated)
		_lobby_table_synchronizer.watch_by_id("w4online.lobby", "id", "updated_at", _id)
		...
		if _type == LobbyType.DEDICATED_SERVER:
		...
		elif _type == LobbyType.WEBRTC_PLAYER_MESH or _type == LobbyType.WEBRTC_PLAYER_HOST or _type == LobbyType.WEBRTC_DEDICATED_SERVER:
			_webrtc_manager.subscribe()

	func unsubscribe() -> void:
		if _lobby_table_synchronizer:
			_lobby_table_synchronizer.unwatch()
			_lobby_table_synchronizer = null
	...
	func _on_lobby_updated() -> void:
		var lobby = _lobby_table_synchronizer.get_record()
		if lobby == null:
			_is_deleted = true
			deleted.emit()
			return
	...
	func _on_precence_updated(p_data: Dictionary) -> void:
		if p_data['type'] == 'INSERT':
			...
			# See supabase realtime GH-213: https://github.com/supabase/realtime/issues/213
			if p_data.get('old_record', {}).get('lobby_id', '') != id:
				return
